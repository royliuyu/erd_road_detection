#######################################################
#  1. Empty Road Detection(ERD) and object detection(yolov5)
# ## senario 1 : with 1st-stage detection with own model
#######################################################
# !/usr/bin/env python
# -*- coding:utf-8 -*-
import warnings
...
###########  setting #########################################
device_s1 = 'cpu'  # when use cpu to execute
device_s2 = 'cuda'
model_name_s2 = 'efficientDet'  # 'ssd' or 'yolov5' or efficientDet
########################################################
    ...
    if is_opened == True:
        ...
        if i % interval == 0:  # interval =2: detect every 2 frames
            ...
            if res_s1 == 1:  # object on the road detected
                ...
                cv2.putText(frame, '%s' % 'Nonempty Road', (780, 50), cv2.FONT_HERSHEY_SIMPLEX, 2,
                            (0, 0, 255), 2)
                if model_name_s2 == 'yolov5':
                ...
                t2 = 0
                cv2.putText(frame, '%s' % 'Empty Road', (780, 50), cv2.FONT_HERSHEY_SIMPLEX, 2, (155, 255, 55),
                            2)
...
data['latency_stage2'] =  latency_s2
data['latency_whole'] =  latency_whole

file_name = 'stage1_erd_'+device_s1+ '+stage2_'+model_name_s2+'_'+device_s2
